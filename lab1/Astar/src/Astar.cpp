...
void Astar_search(const string input_file, int& step_nums, string& way) {
    ...
    while (!open_list.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            new_cell->h = 0; // Heuristic_Funtion();
            ...
            for (int j = 0; j < open_list.size(); j++) {
                if (new_cell->x == open_list.top()->x && new_cell->y == open_list.top()->y) {
                    if (new_cell->g < open_list.top()->g) { // 更新 g 值
                        open_list.pop();
...
int main(int argc, char* argv[]) {
    ...
    // int step_nums = -1;
    // string way = "";
    // Astar_search("../input/input_1.txt", step_nums, way);
    // output("../output/output_1.txt", step_nums, way);
    return 0;
}